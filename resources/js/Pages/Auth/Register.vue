...
function loadRecaptcha(): Promise<void> {
  ...
  return new Promise((resolve, reject) => {
    ...
    script.async = true;
    script.onload = () => (window.grecaptcha ? resolve() : reject(new Error('reCAPTCHA failed to initialize')));
...
async function getRecaptchaToken(): Promise<string> {
  ...
  try {
    ...
    recaptchaError.value = 'reCAPTCHA is not available. Please refresh and try again.';
    return '';
  }
  ...
  try {
    return await new Promise<string>((resolve, reject) => {
      window.grecaptcha!.ready(async () => {
        try {
          const token = await window.grecaptcha!.execute(props.recaptchaSiteKey!, { action: 'register' });
          resolve(token);
        } catch (error) {
          reject(error);
        }
