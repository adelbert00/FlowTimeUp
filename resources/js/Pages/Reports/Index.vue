...
const tagBarChartData = computed(() => ({
  ...
  datasets: [
    {
      label: 'Duration',
      data: tagChartData.value.map((d) => d.duration),
      backgroundColor: '#f43f5e',
    },
    {
      label: 'Earnings',
      data: tagChartData.value.map((d) => d.earnings),
...
const projectBarOptions = computed(() => ({
  ...
  scales: {
    ...
    y: {
      ...
      ticks: {
        callback: (v: unknown) => `${Math.floor(Number(v) / 3600)}h`,
        font: { size: 10 },
      },
    },
  },
...
const projectPieOptions = computed(() => ({
  ...
  plugins: {
    ...
    tooltip: {
      callbacks: {
        label: (ctx: any) => `${ctx.label}: ${formatDuration(Number(ctx.raw))}`,
      },
    },
  },
}));
...
const tagBarOptions = computed(() => ({
  ...
  plugins: {
    ...
    tooltip: {
      callbacks: {
        label: (ctx: any) =>
          ctx.dataset?.label === 'Earnings'
            ? formatCurrency(Number(ctx.raw))
            : formatDuration(Number(ctx.raw)),
      },
    },
  },
  scales: {
...
const updateFilters = () => {
  router.get(
    ...
    {
      project_id:
        filterState.value.project_id === 'all'
          ? null
          : filterState.value.project_id,
      tag_id:
        filterState.value.tag_id === 'all' ? null : filterState.value.tag_id,
...
const exportReport = (format: 'csv' | 'pdf') => {
  ...
  if (filterState.value.project_id !== 'all')
    params.append('project_id', filterState.value.project_id);
  if (filterState.value.tag_id !== 'all')
    params.append('tag_id', filterState.value.tag_id);

  if (filterState.value.start_date)
    params.append('start_date', filterState.value.start_date);
  if (filterState.value.end_date)
    params.append('end_date', filterState.value.end_date);

  params.append(
    'include_billable',
    filterState.value.include_billable.toString()
  );
  params.append(
    'include_non_billable',
    filterState.value.include_non_billable.toString()
  );
    ...
    <div class="space-y-8 animate-fade-up">
      ...
      >
        ...
        <div class="flex items-center gap-3">
          ...
          <Button
            class="bg-red-600 hover:bg-red-700 text-white shadow-lg shadow-red-900/20 transition-all active:scale-95"
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div
          class="bg-surface-raised p-6 rounded-2xl border border-border shadow-sm group hover:border-accent/30 transition-all duration-300"
        >
          <div
            class="text-xs font-bold text-secondary uppercase tracking-[0.15em] mb-2 group-hover:text-accent transition-colors"
          >
            Total Time
          </div>
          <div
            class="text-3xl font-black font-mono tracking-tighter text-primary"
          >
            {{ totalSummary?.total_time || '00:00:00' }}
          </div>
        </div>
        <div
          class="bg-surface-raised p-6 rounded-2xl border border-border shadow-sm group hover:border-accent/30 transition-all duration-300"
        >
          ...
          <div
            class="text-3xl font-black font-mono tracking-tighter text-primary"
          >
            {{ formatCurrency(totalSummary?.total_earnings || 0) }}
          </div>
        </div>
        <div
          class="bg-surface-raised p-6 rounded-2xl border border-border shadow-sm group hover:border-accent/30 transition-all duration-300"
      ...
      <!-- Filters Section -->
      <div
        class="bg-surface-raised rounded-2xl border border-border shadow-sm overflow-hidden relative"
      >
        <div
          class="absolute top-0 right-0 w-32 h-32 bg-accent/5 rounded-full -mr-16 -mt-16 blur-2xl"
        ></div>
        <div class="p-8 relative z-10">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            <div class="space-y-2">
              ...
              <Select v-model="filterState.project_id">
                ...
                <SelectContent
                  class="bg-surface-raised border-border text-primary"
                >
          ...
          <div
            class="flex items-center gap-8 mt-10 pt-8 border-t border-border/50"
          >
            <label class="flex items-center gap-3 cursor-pointer group">
              <div class="relative flex items-center">
                <input
                  type="checkbox"
                  v-model="filterState.include_billable"
                  class="peer sr-only"
                />
                <div
                  class="w-10 h-5 bg-border rounded-full transition-colors peer-checked:bg-accent"
                ></div>
                <div
              ...
              <span
                class="text-xs font-bold text-secondary uppercase tracking-wider group-hover:text-primary transition-colors"
                >Billable</span
              >
            </label>
            <label class="flex items-center gap-3 cursor-pointer group">
              <div class="relative flex items-center">
                <input
                  type="checkbox"
                  v-model="filterState.include_non_billable"
                  class="peer sr-only"
                />
                <div
                  class="w-10 h-5 bg-border rounded-full transition-colors peer-checked:bg-accent"
      ...
      <div class="space-y-6">
        <div
          class="flex bg-surface-raised p-1 rounded-xl border border-border w-fit shadow-sm"
        >
          <button
        ...
        <div
          v-if="activeTab === 'projects' && projectSummary.length > 0"
          class="grid grid-cols-1 md:grid-cols-3 gap-6 animate-scale-in"
        >
          <div
            class="md:col-span-2 bg-surface-raised rounded-2xl border border-border shadow-sm p-6"
          >
            <h4
              class="text-xs font-bold text-secondary uppercase tracking-widest mb-6"
            >
              Duration by Project
            </h4>
          ...
          <div
            class="bg-surface-raised rounded-2xl border border-border shadow-sm p-6 flex flex-col"
          >
            <h4
              class="text-xs font-bold text-secondary uppercase tracking-widest mb-6"
            >
            ...
            <div class="mt-4 grid grid-cols-2 gap-2">
              <div
                v-for="entry in projectChartData.slice(0, 4)"
                :key="entry.name"
              ...
              >
                ...
                ></div>
                <span class="text-[10px] text-secondary truncate font-medium">{{
                  entry.name
                }}</span>
              </div>
            </div>
            ...
            <table class="w-full text-left border-collapse">
              <thead>
                ...
                >
                  ...
                  <th class="px-8 py-5 border-b border-border text-right">
                    Total Duration
                  </th>
                  <th class="px-8 py-5 border-b border-border text-right">
                    Estimated Earnings
              ...
              <tbody class="divide-y divide-border">
                <template v-if="activeTab === 'projects'">
                  ...
                  >
                    <td class="px-8 py-5 font-bold text-primary text-sm">
                      <div class="flex items-center gap-3">
                        <div
                          class="w-2.5 h-2.5 rounded-full shadow-sm ring-2 ring-surface-raised"
                    ...
                    <td
                      class="px-8 py-5 text-right font-mono text-sm text-secondary group-hover:text-primary transition-colors"
                    >
                      {{ formatDuration(item.duration) }}
                    </td>
                    <td
                      class="px-8 py-5 text-right font-mono font-bold text-sm text-primary group-hover:text-accent transition-colors"
                    >
                ...
                <tr
                  v-if="
                    (activeTab === 'projects' && projectSummary.length === 0) ||
                    (activeTab === 'tags' && tagSummary.length === 0)
                  "
                >
                  <td colspan="3" class="px-8 py-20 text-center">
                    <div class="flex flex-col items-center gap-3 opacity-50">
                      ...
                      <p
                        class="text-secondary font-bold text-xs uppercase tracking-widest"
                      >
                        No data found
                      </p>
                    </div>
        ...
        <!-- Tag Breakdown Chart -->
        <div
          v-if="activeTab === 'tags' && tagSummary.length > 0"
          class="bg-surface-raised rounded-2xl border border-border shadow-sm p-8 animate-scale-in"
        >
          <h4
            class="text-xs font-bold text-secondary uppercase tracking-[0.2em] mb-10"
          >
