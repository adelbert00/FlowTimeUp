...
const monthName = computed(() => {
  return dayjs(`${currentYear.value}-${currentMonth.value}-01`).format(
    'MMMM YYYY'
  );
});
  ...
  <MainLayout>
    <div class="space-y-6 animate-fade-up">
      ...
      >
        ...
        <div
          class="flex items-center gap-3 bg-surface-raised p-1 rounded-2xl border border-border shadow-sm"
        >
          <button
            @click="previousMonth"
          ...
          <button
            @click="goToToday"
            class="px-6 py-2 bg-accent text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-accent-hover transition-all shadow-lg shadow-accent/20 active:scale-95"
          >
            Present
          </button>
      ...
      <div
        class="bg-surface-raised rounded-2xl border border-border shadow-sm overflow-hidden"
      >
        <div
        ...
        <div class="p-2 sm:p-6 lg:p-8">
          <div class="grid grid-cols-7 gap-1 sm:gap-3 mb-4 sm:mb-6">
            <div
              v-for="(day, index) in weekDays"
              :key="day"
              class="text-center text-[8px] sm:text-[10px] font-black text-secondary uppercase tracking-[0.15em] sm:tracking-[0.2em]"
            >
          ...
          <div class="grid grid-cols-7 gap-1 sm:gap-3">
            <div
              ...
              :key="index"
              class="min-h-[56px] sm:min-h-[120px] lg:min-h-[140px] p-1.5 sm:p-3 rounded-xl sm:rounded-2xl border transition-all relative group"
              :class="[
                day.isCurrentMonth
                ...
                day.isToday &&
                  'ring-2 ring-accent shadow-lg shadow-accent/10 border-accent/20',
              ]"
            >
              <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-1 sm:mb-3">
                <span
                  class="text-[10px] sm:text-xs font-black w-5 h-5 sm:w-6 sm:h-6 flex items-center justify-center rounded-md sm:rounded-lg transition-colors"
                  :class="[
                    day.isToday ? 'bg-accent text-white' : 'text-primary',
                  ]"
                >
                  {{ day.day }}
                </span>
                <span
                  ...
                  class="text-[7px] sm:text-[9px] font-mono font-black text-accent bg-accent/10 px-1 sm:px-1.5 py-0.5 rounded border border-accent/20 mt-0.5 sm:mt-0 self-start truncate max-w-full"
                >
                  {{ day.totalTime }}
                </span>
              </div>
              ...
              <div class="space-y-1 sm:space-y-1.5 overflow-hidden">
                ...
                <div v-if="day.tasks.length > 0" class="sm:hidden flex flex-wrap gap-0.5 mt-0.5">
                  <span
                    v-for="task in day.tasks.slice(0, 4)"
                    :key="task.id"
                    class="w-1.5 h-1.5 rounded-full"
                    :class="task.completed ? 'bg-muted' : 'bg-accent'"
                    :style="task.project && !task.completed ? { backgroundColor: task.project.color } : {}"
                  />
                  <span v-if="day.tasks.length > 4" class="text-[7px] font-bold text-muted leading-none">+{{ day.tasks.length - 4 }}</span>
                </div>
                <!-- Desktop: full task labels -->
                <div
                  v-for="task in day.tasks.slice(0, 3)"
                  :key="task.id"
                  @click="router.visit(route('tasks.edit', task.id))"
                  class="hidden sm:block text-[9px] font-bold px-2 py-1.5 rounded-lg cursor-pointer truncate transition-all border border-transparent hover:border-white/10"
                  :class="
                    task.completed
                ...
                <div
                  ...
                  class="hidden sm:block text-[9px] font-black text-muted uppercase tracking-widest pl-1 mt-1"
                >
