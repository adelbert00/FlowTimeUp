    ...
    <div class="space-y-6 animate-fade-up">
      ...
      <div class="bg-surface-raised rounded-2xl border border-border p-6 shadow-sm relative overflow-hidden">
        ...
        <div class="flex items-start gap-6 relative z-10">
          <div class="w-12 h-12 rounded-xl bg-accent/10 flex items-center justify-center flex-shrink-0 border border-accent/20">
            <svg class="w-6 h-6 text-accent" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
            </svg>
          </div>
          <div class="flex-1">
            <h3 class="text-lg font-black text-primary uppercase tracking-tighter mb-2">What are Task Templates?</h3>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div class="flex items-start gap-2 p-3 bg-surface-overlay rounded-xl border border-border">
                <svg class="w-4 h-4 text-accent flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/>
                </svg>
                <span class="text-[10px] font-bold text-secondary uppercase tracking-wider"><strong>Save time:</strong> Pre-configure everything</span>
              </div>
              <div class="flex items-start gap-2 p-3 bg-surface-overlay rounded-xl border border-border">
                <svg class="w-4 h-4 text-accent flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/>
                </svg>
                <span class="text-[10px] font-bold text-secondary uppercase tracking-wider"><strong>Consistency:</strong> Uniform structure</span>
              </div>
              <div class="flex items-start gap-2 p-3 bg-surface-overlay rounded-xl border border-border">
                <svg class="w-4 h-4 text-accent flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/>
                </svg>
                <span class="text-[10px] font-bold text-secondary uppercase tracking-wider"><strong>Quick creation:</strong> One-click setup</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div v-if="templates.length === 0" class="flex flex-col items-center justify-center py-20 bg-surface-overlay/10 border border-border border-dashed rounded-2xl">
        ...
        <p class="text-primary font-bold text-lg">No templates yet</p>
        <p class="text-secondary text-sm mb-8">Templates help you automate recurring task creation.</p>
        <button
          @click="router.visit(route('task-templates.create'))"
          class="px-8 py-3 bg-accent text-white rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-accent-hover transition-all shadow-lg shadow-accent/20"
        >
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        >
          <div class="flex items-start justify-between mb-4">
            <h3 class="text-lg font-black text-primary uppercase tracking-tighter group-hover:text-accent transition-colors">{{ template.name }}</h3>
            <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              <button
                @click="router.visit(route('task-templates.edit', template.id))"
                class="p-2 text-muted hover:text-accent hover:bg-surface-overlay rounded-lg transition-all"
              >
          ...
          <div class="space-y-2 mb-6 flex-1">
            <p class="text-sm font-bold text-primary">{{ template.title }}</p>
            <p v-if="template.description" class="text-xs text-secondary line-clamp-2 leading-relaxed">{{ template.description }}</p>
          </div>
          
          <div class="flex items-center gap-2 mb-6">
            <span
              v-if="template.project"
              class="text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded border"
          ...
          <button
            ...
            class="w-full px-4 py-3 bg-accent text-white rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-accent-hover transition-all shadow-lg shadow-accent/20 active:scale-95"
          >
            Use Template
          </button>
        </div>
